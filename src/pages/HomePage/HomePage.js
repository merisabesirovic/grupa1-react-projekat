import React from "react";
import "./HomePage.css";

export default function HomePage() {
  return (
    <div className="homePage">
      <div className="container">
        <h1 className="title">Dobrodosli na nasu stranicu!</h1>
        <h1 className="subtitle">Mi smo najbolja grupa u centru Nit!</h1>
        <p className="smallText">
          Mi smo grupa mladih i ambicioznih ljudi okupljenih u centru Nit,
          posvećeni učenju i razvoju u različitim oblastima tehnologije i
          programiranja. Naša misija je da kroz timski rad, inovacije i
          zajedničko deljenje znanja, pomognemo jedni drugima da napredujemo i
          postignemo naše ciljeve. Ponosni smo na naše projekte, entuzijazam i
          podršku koju pružamo svakom članu tima.
        </p>
      </div>
    </div>
  );
}
